...
class mainObj:
    ...
    def main_func2(self, cutoff, custicker):
        list_of_tickers = yf.Ticker(custicker)
        currentDate = datetime.datetime.strptime(
            date.today().strftime("%Y-%m-%d"), "%Y-%m-%d")
        start_time = time.time()
        d = (self.find_anomalies_two(self.getData(list_of_tickers), cutoff))
        if d['Dates']:
            for i in range(len(d['Dates'])):
                if self.days_between(str(currentDate)[:-9], str(d['Dates'][i])) <= 3:
                    self.customPrint(d, x)
...
# adding new function to either use a custom ticker list
# or just run through the entire charts.

val = input("Would you like to use a custom list of Tickers?(Y/N/y/n): ")
if val == "Y" or "y":
    customlist = input("Enter Ticker list: ")
    mainObj().main_func2(2, customlist)
elif val == "N" or "n":
